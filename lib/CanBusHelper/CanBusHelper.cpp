...
CanBusHelper::CanBusHelper(CONFIG& config, SHIFTER& position) : _config(config),
																_position(position) {
	pinMode(CAN0_INT, INPUT); // CanBus
	pinMode(MOSI1, OUTPUT);   // CanBus
	pinMode(SCK1, OUTPUT);    // CanBus
	pinMode(MISO1, INPUT);    // CanBus
	pinMode(SS1, OUTPUT);     // CanBus

	digitalWrite(SS1, HIGH);  // CanBus
	digitalWrite(SCK1, HIGH); // CanBus
...
void CanBusHelper::buildMessage() {
	for(int i = 0; i < len; i++) {
		canData[i] = ((i == static_cast<int>(_position)) ? 0x01 : 0x00);
	}
	if((_position == SHIFTER::UP) || (_position == SHIFTER::DOWN)) {
		canData[static_cast<int>(SHIFTER::MANUAL)] = 0x01;
	}
}

void CanBusHelper::sendMessage() {
...
void CanBusHelper::setMasks() const {
